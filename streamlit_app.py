...
def enrich(df: pd.DataFrame) -> pd.DataFrame:
    if df.empty:
        return df
    df["date_added"] = pd.to_datetime(df.get("date_added", pd.Series(dtype=str)), errors="coerce", utc=True)
...
def fetch_live():
    ...
    try:
        ...
        for _, row in dfcsv.iterrows():
            records.append({
                ...
                "status":str(row.get("url_status","unknown")),
    ...
    # ThreatFox
    try:
        r = requests.post(THREATFOX_API, json={"query":"get_iocs","days":7}, timeout=30, headers=HDRS)
        r.raise_for_status()
        for e in r.json().get("data",[]):
            if e.get("ioc_type") not in ("url","domain"): continue
            records.append({"source":"threatfox","url":e.get("ioc",""),
                "status":"unknown","date_added":e.get("first_seen",""),
                "threat":e.get("threat_type_desc",""),
                "tags":"|".join(e.get("tags") or []),"host":"","country_code":""})
    except Exception as e3:
        errs.append(f"ThreatFox: {e3}")

    if errs and records:
        st.info(f"ℹ️ Some sources used fallback: {errs[0][:80]}")
